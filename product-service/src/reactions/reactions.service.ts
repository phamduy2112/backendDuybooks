...
export class ReactionsService {
    ...
    async toggleReaction(payloadData){
        ...
        if(existingReaction){
            if(existingReaction.reaction_type===reaction_type){
                return this.prismaService.post_reactions.delete({
                    where:{
                        post_id_user_id:{post_id,user_id},
                ...
                return this.prismaService.post_reactions.update({
                    where:{
                        post_id_user_id:{post_id,user_id},
        ...
        return this.prismaService.post_reactions.create({
            data: { user_id, post_id, reaction_type },
          });
    }
    async getReactionByPost(payloadData){
        return this.prismaService.post_reactions.findMany({
            where:{
                id:payloadData.id
            },
            include:{
    ...
    async getReactionByComment(payloadData){
        return this.prismaService.comment_reactions.findMany({
            where:{
                id:payloadData.id
            },
            include:{
